...
struct KDnode
{
    ...
    KDnode(osg::Vec3f v1, osg::Vec3f v2, osg::Vec3f v3)
        : data(osg::Vec3f(0,0,0))
        , left(NULL)
        , right(NULL) {
...
void printTree(KDnode *root, Trunk *prev, bool isLeft)
{
    ...
    showTrunks(trunk);
    std::cout << "(" << root->data.x() << "," << root->data.y() << "," << root->data.z() << ") " << std::endl;
...
void printVerticalLine(KDnode *node, int line_no, int hd)
{
    ...
    if (hd == line_no) {
        std::cout << "(" << node->data.x() << "," << node->data.y() << "," << node->data.z() << ") ";
    }
...
#define rand_pt(v)                                      \
{                                                       \
    v = KDnode(osg::Vec3f(rand1(), rand1(), rand1()));   \
}
...
BOOST_AUTO_TEST_CASE(KDTree_2d)
{
    ...
    printf(">> WP tree\nsearching for (%g, %g)\n"
           "found (%g, %g) dist %g\nseen %d nodes\n\n",
           testNode.data.x(), testNode.data.y(),
           found->data.x(), found->data.y(), sqrt(best_dist), visited);
...
BOOST_AUTO_TEST_CASE(KDTree_3d)
{
    ...
    printf("\n>> Million tree\nsearching for (%g, %g, %g)\n"
           "found (%g, %g, %g) dist %g\nseen %d nodes\n",
           testNode.data.x(), testNode.data.y(), testNode.data.z(),
           found->data.x(), found->data.y(), found->data.z(),
