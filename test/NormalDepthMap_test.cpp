...
BOOST_AUTO_TEST_CASE(applyShaderNormalDepthMap_TestCase) {
    ...
    for (uint i = 0; i < eyes.size(); i++) {
        ...
        cv::Mat rawShader = computeNormalDepthMap(
                                                root, maxRange, fovX, fovY,
                                                0, eyes[i], centers[i], ups[i]);
...
void checkDepthValueRadialVariation(cv::Mat3f image, uint id) {
    ...
    groundTruthVector[0] = std::vector<int>(groudTruth0, groudTruth0
                                           + sizeof(groudTruth0) / sizeof(int));
    groundTruthVector[1] = std::vector<int>(groudTruth1, groudTruth1
                                           + sizeof(groudTruth1) / sizeof(int));
    groundTruthVector[2] = std::vector<int>(groudTruth2, groudTruth2
                                           + sizeof(groudTruth2) / sizeof(int));
...
BOOST_AUTO_TEST_CASE(depthValueRadialVariation_testCase) {
    ...
    for (uint j = 0; j < sizeVector; ++j) {
        ImageViewerCaptureTool capture(fovys[j] * M_PI / 180.0, fovxs[j]
                                                * M_PI / 180.0, heightSize[j]);
