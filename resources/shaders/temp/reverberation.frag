...
void main() {
    ...
    // ivec2 texDim = textureSize(vertexMap, 0);
    // vec3 value = texture(vertexMap, gl_FragCoord.xy / vec2(512,512)).xyz;
    // vec3 value = texture(vertexMap, gl_FragCoord.xy / texDim).xyz;

    // vec3 value = texelFetch(vertexMap, texDim, 0).xyz;
